...
void SystemSub::NodeListSub::startNode(string topicName)
{
    // 节点初始状态进入活动状态
...
void SystemSub::NodeListSub::stopNode(string topicName)
{
    // 节点从活动状态返回初始状态
    topicName_ = topicName;
...
void SystemSub::NodeListSub::deactivateNode(string topicName)
{
    // 节点从活动状态返回初始状态
    topicName_ = topicName;
...
SystemSub::MapSub::MapSub(const string &node_name, QObject *parent) : QObject(parent), rclcpp_lifecycle::LifecycleNode(node_name)
{
    qRegisterMetaType<QVector<QPointF>>("QVector<QPointF>");
    qRegisterMetaType<QVector<int>>("QVector<int>");
}

CallbackReturn SystemSub::MapSub::on_configure(const rclcpp_lifecycle::State &state)
{
    ...
    local_path_pub_ = this->create_subscription<nav_msgs::msg::Path>(
        "/local_plan", 10,
        std::bind(&SystemSub::MapSub::localPathCallback, this, std::placeholders::_1));
    global_path_pub_ = this->create_subscription<nav_msgs::msg::Path>(
        "/plan", 10,
        std::bind(&SystemSub::MapSub::globalPathCallback, this, std::placeholders::_1));
    map_sub_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        "/map", 1,
        std::bind(&SystemSub::MapSub::mapCallback, this, std::placeholders::_1));

    RCLCPP_INFO(this->get_logger(), "SysSub::on_configure() is called.");
    return CallbackReturn::SUCCESS;
}
CallbackReturn SystemSub::MapSub::on_activate(const rclcpp_lifecycle::State &state)
{
    RCLCPP_INFO(this->get_logger(), "SysSub::on_activate() is called.");
    return CallbackReturn::SUCCESS;
}
CallbackReturn SystemSub::MapSub::on_deactivate(const rclcpp_lifecycle::State &state)
{
...
CallbackReturn SystemSub::MapSub::on_shutdown(const rclcpp_lifecycle::State &state)
{
    RCLCPP_INFO(this->get_logger(), "SysSub::on_shutdown() is called.");
    return CallbackReturn::SUCCESS;
}
CallbackReturn SystemSub::MapSub::on_cleanup(const rclcpp_lifecycle::State &state)
{
    RCLCPP_INFO(this->get_logger(), "SysSub::on_shutdown() is called.");
    return CallbackReturn::SUCCESS;
}

void SystemSub::MapSub::startNode()
{
    // 节点初始状态进入活动状态
    RCLCPP_INFO(this->get_logger(), "节点初始状态进入活动状态");
    this->trigger_transition(lifecycle_msgs::msg::Transition::TRANSITION_CONFIGURE);
...
void SystemSub::MapSub::stopNode()
{
    // 节点从活动状态返回初始状态
...
void SystemSub::MapSub::deactivateNode()
{
    // 节点从活动状态返回初始状态
    amcl_initialized_ = false;
    RCLCPP_INFO(this->get_logger(), "销毁节点");
...
void SystemSub::MapSub::onmousePress(double x, double y, double yaw)
{
...
void SystemSub::MapSub::onInitializeAMCL(int flog)
{
    if (flog == 1)
    {
...
void SystemSub::MapSub::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
...
void SystemSub::MapSub::localPathCallback(const nav_msgs::msg::Path::SharedPtr msg)
{
...
void SystemSub::MapSub::globalPathCallback(const nav_msgs::msg::Path::SharedPtr msg)
{
    QVector<QPointF> path_points;
    for (const auto &pose_stamped : msg->poses)
    {
