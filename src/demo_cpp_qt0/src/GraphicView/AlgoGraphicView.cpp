...
AlgoGraphicView::AlgoGraphicView(QWidget *parent) : QGraphicsView(parent)
{
	...
	action = new QAction(tr("设置为导航点"), this);
	addAction(action);
	this->connect(action, SIGNAL(triggered(bool)), this, SLOT(onSetNavPoint()));
...
void AlgoGraphicView::onSetNavPoint()
{
	qDebug() << "onSetNavPoint";
	emit newNavPointSignal(m_curMousePos);
}
